...
class LaporanTopPasienNominalTerbanyakExportService
{
    public function pdf(array $report): Response
    {
        $pdf = Pdf::loadView('laporan.top-pasien-nominal-terbanyak.export', [
            'report' => $report,
    ...
    public function excel(array $report): StreamedResponse
    {
        ...
        $spreadsheet->getProperties()
            ...
            ->setSubject('Laporan top pasien berdasarkan nominal transaksi')
            ->setDescription('Peringkat pasien berdasarkan akumulasi grand total invoice lunas.');

        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setTitle('Top Nominal');
        ...
        foreach ($report['rows'] as $item) {
            ...
            $sheet->setCellValue("C{$row}", (float) $item['nominal']);
            $sheet->setCellValue("D{$row}", (int) $item['total_transaksi']);
        ...
        if (empty($report['rows'])) {
            ...
            $sheet->setCellValue(
                "A{$row}",
                'Tidak ada data pasien pada periode dan range nominal yang dipilih.'
            );
