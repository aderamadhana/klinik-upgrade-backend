...
class LaporanPasienTreatmentTerbanyakExportService
{
    public function pdf(array $report): Response
    {
        $pdf = Pdf::loadView('laporan.pasien-treatment-terbanyak.export', [
            'report' => $report,
    ...
    public function excel(array $report): StreamedResponse
    {
        ...
        $spreadsheet->getProperties()
            ...
            ->setSubject('Laporan pasien treatment terbanyak')
            ->setDescription('Peringkat pasien berdasarkan total qty treatment, jumlah transaksi, dan total nominal treatment net.');

        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setTitle('Pasien Treatment');
        ...
        if (empty($report['rows'])) {
            ...
            $sheet->setCellValue(
                "A{$row}",
                'Tidak ada data pasien treatment pada periode dan filter yang dipilih.'
            );
