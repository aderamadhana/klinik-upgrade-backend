...
<head>
    ...
    <style>
        ...
        * {
        ...
        .report-table th {
            background: #d9e2f3;
            border-bottom: 2px solid #d8a900;
            color: #111827;
            font-size: 7.8px;
            font-weight: bold;
            text-align: center;
    ...
    <table class="report-table">
        <thead>
            <tr>
                ...
                <th style="width: 17%;">Kode Bahan</th>
                <th style="width: 48%;">Nama Bahan</th>
                <th style="width: 10%;">Satuan</th>
                <th style="width: 11%;">Total Jml</th>
                <th style="width: 7%;">Frek.</th>
            </tr>
        </thead>
        <tbody>
            @forelse ($report['groups'] as $treatment)
                ...
                @foreach ($treatment['items'] as $item)
                    <tr class="keep-row">
                        ...
                        <td class="right">{{ $formatQty($item['total_jumlah']) }}</td>
                        <td class="center">{{ number_format((int) $item['frekuensi'], 0, ',', '.') }}</td>
                    </tr>
                @endforeach
            @empty
                <tr class="empty-row">
                    <td colspan="6">Tidak ada penggunaan bahan treatment pada periode ini.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
