...
<body>
    ...
    <div class="filter-note">
        ...
        | Range nominal {{ $report['nominal_range_label'] }}
        ...
        <tbody>
            @forelse ($report['rows'] as $item)
                <tr>
                    ...
                    <td class="money-cell">{{ $money($item['nominal']) }}</td>
                    <td class="number-cell">{{ number_format((int) $item['total_transaksi'], 0, ',', '.') }}</td>
                </tr>
            @empty
                <tr class="empty-row">
                    <td colspan="4">Tidak ada data pasien pada periode dan range nominal yang dipilih.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
